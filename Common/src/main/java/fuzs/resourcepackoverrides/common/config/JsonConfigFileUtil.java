...
public class JsonConfigFileUtil {
    ...
    public static boolean copyToFile(File jsonFile) {
        ...
        try (InputStream input = JsonConfigFileUtil.class.getResourceAsStream(
                "/" + jsonFile.getName()); FileOutputStream output = new FileOutputStream(jsonFile)) {
            if (input != null) {
            ...
            ResourcePackOverrides.LOGGER.error("Failed to copy {} in config directory: {}", jsonFile.getName(), e);
        }
        return false;
    }
    ...
    public static boolean saveToFile(File jsonFile, JsonElement jsonElement) {
        ...
        try (FileWriter writer = new FileWriter(jsonFile)) {
            ...
            ResourcePackOverrides.LOGGER.error("Failed to create {} in config directory: {}", jsonFile.getName(), e);
        }
        return false;
    }
            ...
            for (File file : allFilesAndDirs) {
                if (file.isDirectory()) {
                    ...
                    try {
                        ...
                        ResourcePackOverrides.LOGGER.error("Failed to locate files in {} directory: {}",
                                directory.getName(),
                                e
                        );
                    }
